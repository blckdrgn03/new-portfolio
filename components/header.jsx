    ...
    <Sheet>
      ...
      <SheetContent>
        ...
        <div className="flex h-screen flex-col justify-center items-center gap-8 py-4">
            {links.map((link, i) => {
                        return <a className={`${currentPath == link.path && "text-accent border-accent border-b-2"} hover:text-accent transition-all duration-300 capitalize text-2xl md:text-3xl`}  key={i} href={link.path}>{link.name}</a>;
                    })}
        </div>
